...
func (tx *Tx) init() {
	tx.MiniProgramAccount = NewMiniProgramAccountClient(tx.config)
	tx.PhoneAccount = NewPhoneAccountClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.UserAccount = NewUserAccountClient(tx.config)
	tx.UserExtend = NewUserExtendClient(tx.config)
...
// applies a query, for example: MiniProgramAccount.QueryXXX(), the query will be executed
